// Position-asserting layout tests for the treemap (Tree of Life view).
// Area-only tests pass on visually broken layouts — assert bounds + overlap.
...
describe('squarify — positional correctness', () => {
  ...
  const valueSets: number[][] = [
    ...
    Array.from({ length: 120 }, () => 1 + Math.floor(rand() * 200)),
  ];
